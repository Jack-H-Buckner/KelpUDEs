...
function init_model(dat,X,K,mean_t1,mean_t2,K1,K2,padding;hidden = 10,seed = 123)

    n_sites = size(dat[:,2:end])[2]
    
    NN, NNparams = UniversalDiffEq.SimpleNeuralNetwork(2+2*padding+1,1,hidden = hidden, seed = seed)
    ...
    function dudt(u,X,p,t)
        ...
        for i in 1:n_sites
            ...
            if i <= padding
                ...
                inputs = vcat(u[i:i],u_.+log.(K_),X[i])
            ...
            du_i = NN(inputs,p.NN)[1]*(1-exp(u[i])) .- abs(p.FE[i])
            du = vcat(du,du_i)  
        end
        return du
    end
    ...
    model = UniversalDiffEq.CustomDerivatives(dat,X,dudt,init_params,time_column_name = "year",variable_column_name = "variable", value_column_name = "value")
...
function init_null_model(dat)
    ...
    model = UniversalDiffEq.CustomDerivatives(dat,dudt,init_params,time_column_name = "year")

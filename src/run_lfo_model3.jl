...
dat = CSV.read("processed_data/kelp_bins_50_500m_cenca_model_2.csv",DataFrame)[:,vcat([2],21:30)]
dat1 = CSV.read("processed_data/kelp_bins_50_500m_cenca_model_2.csv",DataFrame)[:,20]
dat2 = CSV.read("processed_data/kelp_bins_50_500m_cenca_model_2.csv",DataFrame)[:,31]
dat[:,2:end] .= log.(dat[:,2:end] .+ 1e-2)


# Cell 2
dat_K = CSV.read("processed_data/kelp_bins_50_500m_cenca_model_2_K.csv",DataFrame)[:,21:20]
K1 = CSV.read("processed_data/kelp_bins_50_500m_cenca_model_2_K.csv",DataFrame)[:,20]
...
UniversalDiffEq.leave_future_out(model, x->training!(x,1e1,0.05), 10; 
                            path = string("results/cv/model2_1e1_0.05.csv"))

print("run 1")
UniversalDiffEq.leave_future_out(model, x->training!(x,1e5,0.05), 10; 
                            path = string("results/cv/model2_1e5_0.05.csv"))

print("run 2")
UniversalDiffEq.leave_future_out(model, x->training!(x,1e6,0.05), 10; 
                            path = string("results/cv/model2_1e6_0.05.csv"))

print("run 3")
UniversalDiffEq.leave_future_out(model, x->training!(x,1e7,0.05), 10; 
                            path = string("results/cv/model2_1e7_0.05.csv"))

print("run 4")                            
UniversalDiffEq.leave_future_out(model, x->training!(x,1e8,0.05), 10; 
                            path = string("results/cv/model2_1e8_0.05.csv"))

print("run 5")
UniversalDiffEq.leave_future_out(model, x->training!(x,1e9,0.05), 10; 
                            path = string("results/cv/model2_1e9_0.05.csv"))
